...
H = 0.01; // height of cylinder
nb_slices_quarter = 64; //number of layers in one quarter OF CIRCLE
...
For i In {1:n_points1}
 ...
 If(i==9||i==17||i==25||i==33||i==41||i==49||i==57) //for n_points=65;
   r = r+0.001;
  EndIf
  pList_1[i] = newp;
...
Transfinite Line {25,26,27,28} = 30 Using Progression 1.05;
...
Extrude {0, 0, H} {
  Surface{14, 32}; Layers{1}; Recombine;
}
Coherence;
...
Physical Surface(1) = {82, 60};
Physical Surface(2) = {59};
Physical Surface(3) = {73};
Physical Surface(4) = {55,77};
Physical Surface(5) = {47, 69};

Physical Volume(0) = {2, 1};
